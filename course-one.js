(() => {
        ...
        try {
          ...
          data.items.forEach((it, idx) => {
            ...
            it.options.forEach((opt,i)=>{
              ...
              r.addEventListener('change', () => { it._choice=i; setBtnState(submit, data.items.every(x=>x._choice!==null)); });
          ...
          submit.addEventListener('click', () => {
            ...
            data.items.forEach((it, idx) => {
              ...
              card.classList.add(ok ? 'correct' : 'incorrect');
              const res = card.querySelector('.result');
              res.innerHTML = `${ok ? 'Correct.' : 'Incorrect.'} Recommended counter-move: ${it.countermove}<div class="drawer">Rationale: ${it.rationale}</div>`;
            });
            out.textContent = `Completed ${data.items.length} items.`;
            state.m2.idExercise = true; saveState(state); updateLocks();
            ffTrack('id_exercise_complete', { items:data.items.length, correct });
    ...
    function updateLocks(){
      ...
      // Lock everything else — but NEVER re-lock the same element if multiple steps share it
      LINEAR_STEPS.forEach((s) => {
        const el = document.querySelector(s.section);
        if (!el) return;
        if (el === currentEl) return;
  
        // Keep pre-quiz visible (read-only) after completion
